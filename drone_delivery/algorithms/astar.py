"""A* search: f(n) = g(n) + h(n), with per-edge battery and charging stations."""
...
def astar(
    ...
    gen = astar_steps(graph, start, goal, heuristic, drone, charging_stations)
...
def default_astar_steps(start: str, goal: str, heuristic: dict[str, int], drone):
    return astar_steps(GRAPH, start, goal, heuristic, drone, set(BATTERY_STATIONS))
